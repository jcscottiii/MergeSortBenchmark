...
MergeSortBenchmarkApp/app/src/main/jniLibs/armeabi-v7a/libgomergesort.so: go_gomergesort/go_gomergesort.go
	cd go_gomergesort_so/ && CGO_ENABLED=1 GOOS=android GOARCH=arm go build -o libgomergesort.so -ldflags="-shared" . && cd ../
	mkdir -p MergeSortBenchmarkApp/app/src/main/jniLibs/armeabi-v7a/
	mv go_gomergesort_so/libgomergesort.so MergeSortBenchmarkApp/app/src/main/jniLibs/armeabi-v7a/libgomergesort.so
...
MergeSortBenchmarkApp/app/src/main/java/go/gomergesort/Gomergesort.java:
	...
	CGO_ENABLED=0 gobind -lang=java . > MergeSortBenchmarkApp/app/src/main/java/go/gomergesort/Gomergesort.java

go_gomergesort/go_gomergesort.go:
	mkdir -p go_gomergesort
	CGO_ENABLED=0 gobind -lang=go . > go_gomergesort/go_gomergesort.go
